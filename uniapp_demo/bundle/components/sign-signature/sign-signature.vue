<!--签名 <Signature ref="sign" v-model="signValue"></Signature> -->
<template>
	<view class="signature-wrap">
		<view class="img-wrap" @tap="showSignature()" @touchstart="touchSignature()">
			<text class="tips" v-if="prevView == ''">点击此处签名确认</text>
		...
		<view v-if="!disabled" v-show="show" class="signature-contain">
			<view class="signature-main" style="z-index: 3000;">
				...
				<view class="signature-title">
					<text v-for="(t, index) in titles" :key="index">{{t}}</text>
				</view>
				...
				<view class="signature-btns">
					...
					<view class="btn btn-ok cu-btn bg-main margin-tb-sm text-white" @tap="saveSign()">
