					...
					<view class="cal-ul cal-days">
						<view class="cal-li" v-for="(item, j) of days" :key="j" @click="changeSelected(item)">
							<view
								...
								:style="{ color: item.ymd < baseData.currentDate.ymd ? '#999' : '#333' }"
							>
								<text>{{ item.d }}</text>
							</view>
						</view>
...
export default {
	...
	methods: {
		...
		initData() {
			...
			this.$set(this.baseData, 'currentDate', {
				d: nowSat.getDate(),
				ymd: `${lastSat.getFullYear()}${this.forMatNumber(nowSat.getMonth() + 1)}${this.forMatNumber(nowSat.getDate())}`,
				timeSpan: +nowSat,
				m: new Date(nowSat),
				disabled:true,
			});
			...
			this.$set(this.baseData, 'selectedDate', {
				d: lastSat.getDate(),
				ymd: `${lastSat.getFullYear()}${this.forMatNumber(lastSat.getMonth() + 1)}${this.forMatNumber(lastSat.getDate())}`,
				timeSpan: +lastSat,
				m: new Date(lastSat),
				disabled:lastSat.getDate() < nowSat.getDate(),
			});
		...
		changeWeek(type) {
			...
			console.log('修改周current',this.current);
		},
		// 通过周六数据获取一周的数据
