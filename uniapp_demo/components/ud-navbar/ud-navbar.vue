<template>
	<view class="ud-navigation-bar"
		:class="{ 'ud-bar-line': opacity > 0.85 && splitLine, 'ud-navbar-fixed': isFixed, 'ud-backdrop__filter': backdropFilter && dropDownOpacity > 0  }"
		...
		<view class="ud-status-bar" :style="{ height: statusBarHeight + 'px' }" v-if="isImmersive"></view>
		<view class="ud-navigation_bar-title"
			:style="{ 
...
<script>
	export default {
		...
		onReady() {
			this.computedFontSize(this.title);
		},
		methods: {
			computedFontSize(val){
