	...
	<view class="nav-bar" :style="'height: calc('+statusBarHeight+'px + '+topHeight+'rpx)'">
		...
		<!-- #endif -->
			...
			<view class="topMain" :style="'height: '+topHeight+'rpx'">
				<!-- 返回 -->
				<view @click="goBack" class="back" v-if="back || !center">
					<!-- <u-icon name="arrow-left" color="#fff"></u-icon> -->
					<image class="img" :src="'/static/images/ico_back.png'"></image>
				</view>
				<slot></slot>
				<!-- 标题文字 -->
				<text v-if="!center" class="title-left" :style="{'color': titleColor}">{{title}}</text>
				<text v-else class="title" :style="{'color': titleColor}">{{title}}</text>
