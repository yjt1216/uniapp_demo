<template>
	<view class="page">
		...
		<!-- #endif -->
		<view class="input">
			<textarea style="font-size: 16px;" v-model="text" :auto-height="true" :maxlength="150">
			</textarea>
		</view>
		...
		<scroll-view :scroll-y="true" style="height: 500px;">
			<view class="page">
				<view class="qrcode-box">
					<view class="qrcode">
						<uqrcode ref="qr1" canvas-id="qr1" :value="text" size="200" :start="true" 
							:auto="true" @click="remake('qr1')" @complete="complete($event, 'qr1')">
						</uqrcode>
					</view>
					<text class="msg">这是一个常规的二维码</text>
					<view class="btns">
						<button class="btn" type="primary" size="mini" @click="save('qr1')">
							保存
						</button>
					</view>
				</view>
				<view class="qrcode-box">
					...
					<text class="msg">
						这是一个不使用useDynamicSize选项参数的二维码，因为canvas的特性，会出现0.5px的绘制误差，
						组件是默认开启useDynamicSize的
					</text>
					<view class="btns">
						<button class="btn" type="primary" size="mini" @click="save('qr2')">
							保存
						</button>
					</view>
				</view>
