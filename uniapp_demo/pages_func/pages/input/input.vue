<template>
	<view>
		<u-navbar title="输入框" :auto-back="true" :placeholder="true"></u-navbar>
		...
		<uni-section title="默认" subTitle="使用 focus 属性自动获取输入框焦点" type="line" padding>
			<uni-easyinput errorMessage v-model="defaultValue" focus placeholder="请输入内容" @input="defaultInput"></uni-easyinput>
		</uni-section>
		...
		<uni-section title="自定义样式" subTitle="使用 styles 属性 ,可以自定义输入框样式" type="line" padding>
			<uni-easyinput v-model="confirmValue" :styles="styles" :placeholderStyle="placeholderStyle" placeholder="请输入内容" 
				@input="confirmInput" @confirm="confirmFun">
			</uni-easyinput>
		</uni-section>
...
<script>
	export default {
		...
		methods: {
			...
			defaultInput(e){
				console.log('默认输入内容e：', e);
				console.log('默认输入内容defaultValue：', this.defaultValue);
			},
			confirmInput(e){
				console.log('confirm输入内容e：', e);
				console.log('confirm输入内容defaultValue：', this.defaultValue);
			},
			iconClick(type) {
				uni.showToast({
			...
			confirmFun(){
				console.log('confirm提交输入内容：', this.confirmValue);
				this.confirmValue = "";
				console.log('confirm清空内容：', this.confirmValue);
			},
